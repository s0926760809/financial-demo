...
# 啟動 Trading API 服務
start_trading_api() {
    print_step "啟動 Trading API 服務..."
    
    # 檢查端口是否被佔用
    if lsof -i :30080 &> /dev/null; then
        print_warning "Trading API 端口 30080 已被佔用，可能已經在運行"
        return
    fi
    ...
    if [ $? -eq 0 ]; then
        ...
        if curl -s http://localhost:30080/health > /dev/null; then
            ...
            print_error "Trading API 啟動失敗，檢查日誌: logs/trading-api.log"
        fi
    else
...
start_frontend() {
    ...
    if lsof -i :5173 &> /dev/null; then
        print_warning "前端端口 5173 已被佔用，可能已經在運行"
        return
    fi
...
main() {
    ...
    echo ""
    echo -e "${GREEN}🎉 啟動完成！開始使用您的金融交易系統吧！${NC}"
    echo -e "${CYAN}💡 現在可以在前端界面創建 GOOGL 訂單，它們會立即成交！${NC}"
}
